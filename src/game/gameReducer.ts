import { GameState, Unit, Position, posEqual, posKey, rollDice, getAdjacent, getAdjacentEnemies, isIn5x5, CorpseMarker, createUnit } from './types';
...
function applyDamage(state: GameState, targetId: string, damage: number, source: string): GameState {
  ...
  // Banner of Iron Faith damage reduction
...
function nextActivation(state: GameState): GameState {
  ...
  // Other player has no units left to activate, stay with current
...
function endRound(state: GameState): GameState {
  ...
  // Remove corpse markers, reset abilities, clear pinned
  ...
  return {
    ...
    units: state.units.map(u => ({
      ...
      pinned: u.pinned ? false : u.pinned, // clear pinned after skipping
    })),
  };
}
  ...
  switch (action.type) {
    case 'START_GAME':
      return {
        ...
        phase: 'hazard_placement',
        hazardsToPlace: 3,
        log: ['Welcome to SLATRA. Place 3 hazard tiles in rows 3-6.'],
      };
    ...
    case 'ROLL_OBJECTIVES': {
      ...
      let col1 = roll1.total - 1; // P2's objective in row 5
      let col2 = roll2.total - 1; // P1's objective in row 4

      // Ensure not on hazards
      while (state.hazards.some(h => h.position.row === 5 && h.position.col === col1)) {
        col1 = (col1 + 1) % 6;
      }
      while (state.hazards.some(h => h.position.row === 4 && h.position.col === col2)) {
        col2 = (col2 + 1) % 6;
      }
      ...
      let newState: GameState = {
        ...
        phase: 'deployment_p1',
        objectives: [
          { position: { row: 5, col: col1 }, faction: 'bone', type: 'bone_altar', used: false },
          { position: { row: 4, col: col2 }, faction: 'plague', type: 'plague_crate', used: false },
        ],
      };
      newState = addLog(newState, `Bone Altar placed at 5${['A','B','C','D','E','F'][col1]} (rolled ${roll1.total})`);
      newState = addLog(newState, `Plague Crate placed at 4${['A','B','C','D','E','F'][col2]} (rolled ${roll2.total})`);
      newState = addLog(newState, 'Plague Order: Deploy your 6 units in rows 1-2.');
      return newState;
    }
    ...
    case 'DEPLOY_UNIT': {
      ...
      // Determine which unit to place next
      const factionUnits = state.units.filter(u => u.faction === faction);
      const gruntCount = factionUnits.filter(u => u.unitClass === 'grunt').length;
      const hasMedic = factionUnits.some(u => u.unitClass === 'medic');
      const hasHeavy = factionUnits.some(u => u.unitClass === 'heavy');
      const hasCaptain = factionUnits.some(u => u.unitClass === 'captain');

      let unitClass: 'grunt' | 'medic' | 'heavy' | 'captain';
      let index = 0;
      if (gruntCount < 3) { unitClass = 'grunt'; index = gruntCount; }
      else if (!hasMedic) { unitClass = 'medic'; }
      else if (!hasHeavy) { unitClass = 'heavy'; }
      else if (!hasCaptain) { unitClass = 'captain'; }
      else return state; // All deployed
      ...
      if (newState.units.filter(u => u.faction === faction).length >= 6) {
        if (faction === 'plague') {
          newState = { ...newState, phase: 'deployment_p2' };
          newState = addLog(newState, 'Bone Legion: Deploy your 6 units in rows 7-8.');
        } else {
          newState = { ...newState, phase: 'initiative_roll' };
    ...
    case 'MOVE_UNIT': {
      ...
      // Check path is clear (simple: just check destination for now)
    ...
    case 'ATTACK_UNIT': {
      ...
      // Filth Scorch (Plague Soldier kills)
      ...
      // Ancient Evasion check (Bone Legion grunts)
      ...
      if (updatedTarget2 && updatedTarget2.hp > 0 && updatedTarget2.faction === 'bone' && updatedTarget2.unitClass === 'grunt' && !updatedTarget2.usedReactionThisRound && !updatedTarget2.pinned) {
        // Auto-trigger: find a valid escape tile
      ...
      if (attacker.faction === 'bone' && attacker.unitClass === 'heavy') {
        ...
        if (pushTarget && pushTarget.hp > 0) {
          ...
          if (pushDest.row >= 1 && pushDest.row <= 8 && pushDest.col >= 0 && pushDest.col <= 5 && !occupiedSet2.has(posKey(pushDest))) {
            ...
            // Check hazard on push destination
    ...
    case 'USE_ABILITY': {
      ...
      // Handle abilities per unit type
      ...
      if (unit.unitClass === 'medic' && unit.faction === 'bone' && action.targetId === 'resurrect') {
        ...
        resUnit.activated = true; // Can't act this round
        newState = {
          ...newState,
    ...
    case 'END_ACTIVATION': {
      ...
      // Check hazard damage if ending on hazard
